...
class GFlasso:
    def __init__(self, X=None, y=None, corr_coff=None, beta=None, lambda_flasso=1., gamma_flasso=0.7, flasso_type=0,
                 edge_vertex_matrix=None, mau=0.1, alpha_matrix=None, L1=0.):
        self.X = X
        self.y = y
        ...
        for i in range(0, num_rows):
            for j in range(0, num_cols):
                ...
                total_sum = total_sum + mul_factor * abs(
                    (self.beta[:, i]).sum() - sign * (self.beta[:, j]).sum())
        return total_sum
